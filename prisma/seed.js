...
const comprehensiveCoursesByDepartment = {
    'Computer Science': [
        // 100 Level
...
async function seedStudents() {
    ...
    // Helper functions
    const generateMatricNo = (year, deptCode, index) => {
    ...
    for (const department of depts) {
        ...
        for (let levelIndex = 0; levelIndex < levels.length; levelIndex++) {
            ...
            for (let i = 0; i < studentsForLevel; i++) {
                ...
                const enrollmentDate = getRandomDate(new Date(2023, 8, 1), // Sept 1, 2023
                new Date(2025, 11, 31) // Dec 31, 2025
                );
                // Random date of birth (18-25 years old)
                const dateOfBirth = getRandomDate(new Date(1998, 0, 1), new Date(2006, 11, 31));
                // Random address
    ...
    if (remaining > 0) {
        ...
        for (let i = 0; i < remaining; i++) {
            ...
            const enrollmentDate = getRandomDate(new Date(2023, 8, 1), new Date(2025, 11, 31));
            const dateOfBirth = getRandomDate(new Date(1998, 0, 1), new Date(2006, 11, 31));
            const address = `${Math.floor(Math.random() * 100) + 1} ${getRandomElement(lastNames)} Street, ${getRandomElement(cities)}`;
            try {
                await prisma.student.create({
